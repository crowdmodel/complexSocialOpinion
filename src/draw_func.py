...
def show_geom(simu, debug=False):
    ...
    while running: 
            ...
            if event.type == pygame.QUIT:
                    ...
                    menu_01 = not menu_01
                     ...
                     if  menu_01:
                         if mouseX<120 and mouseX>0 and mouseY<40 and mouseY>23:
                             ...
                             updateExit2Doors(exit2door, os.path.join(simu.fpath, 'bldDataRev.csv'), message='in testGeom:')
                             menu_01 =False
                ...
                if mouseX>=0 and mouseX<=500 and mouseY>=0 and mouseY<=20:
                    ...
                    if menu_05 is True:
                        ...
                        print('length of doors list:', len(doors))
                    ...
                    if menu_06 is True:
                        ...
                        geomRev = True
                        print('Direction of the exit added:', exits[-1].arrow)
                        print('length of exits list:', len(exits))
        ...
        text_surface=myfont.render("zmin:"+str(simu.zmin), True, white, black)
        screen.blit(text_surface, [726.0, 395.0])
        text_surface=myfont.render("zmax:"+str(simu.zmax), True, white, black)
        screen.blit(text_surface, [726.0, 415.0])

        text_surface=myfont.render("num_agents:"+str(len(agents)), True, white, black)
        screen.blit(text_surface, [726.0, 439.0])
        text_surface=myfont.render("num_walls:"+str(len(walls)), True, white, black)
        screen.blit(text_surface, [726.0, 459.0])

        text_surface=myfont.render("num_doors:"+str(len(doors)), True, white, black)
        screen.blit(text_surface, [726.0, 479.0])
        text_surface=myfont.render("num_exits:"+str(len(exits)), True, white, black)
        screen.blit(text_surface, [726.0, 496.0])
...
def show_flow(simu):
    ...
    while running:
        ...
        # Show Mouse Absolute and Relative Positions on the Screen
        myfont=pygame.font.SysFont("arial",16)
        xycoordinate=(mouse_pos2-xyShift)*(1/ZOOMFACTOR)
        text_surface=myfont.render(str(xycoordinate), True, black, white)
        ...
        screen.blit(text_surface, mouse_pos2+[0.0, 56.0])
    ...
    while running:
        ...
        for idai, agent in enumerate(simu.agents):
            ...
            if simu.t_sim < agent.tpre:
                try:
                    pygame.draw.circle(screen, black, scPos, int(agent.radius*ZOOMFACTOR), LINEWIDTH)
                except:
